#################################
### FRE7241 Test #2 Solution Tuesday February 10
#################################
# Max score 80pts
...
colnames(volumv) <- c("volume")
volv <- (quantmod::Hi(ohlc) - quantmod::Lo(ohlc))
colnames(volv) <- c("volat")
...
mean(volumv[timev == timu[10]])
# Or
mean(ohlc[timev == timu[10], "SPY.Volume"])
...
volav <- sapply(timu, function(x) {
  selv <- (timev == x)
  c(volume=mean(volumv[selv]),
    volat=mean(volv[selv]))
})  # end sapply
volav <- t(volav)
...
colv <- colnames(volav)
dygraphs::dygraph(volav, main="Intraday Volatility and Trading Volumes") %>%
...
formulav <- as.formula(paste(colnames(volav), collapse=" ~ "))
...
plot(formula=formulav, data=volav,
     main="Scatterplot of Volume versus Volatility")
abline(regmod, lwd=3, col="red")
text(x=median(volv), y=7*median(volumv), cex=1.1, col="blue", 
     labels=paste("t-value =", round(with(regsum, coefficients[2, 3]), 2)))
...
regmod <- lm(formulav, data=voldiff)
...
plot(formula=formulav, data=voldiff,
     main="Scatterplot of Differences in
            Volume versus Volatility")
abline(regmod, lwd=3, col="red")
text(x=max(voldiff$volat)/2, y=min(voldiff$volume)/2, cex=1.1, col="blue", 
     labels=paste("t-value =", round(with(regsum, coefficients[2, 3]), 2)))
