...
fridays <- which(.indexwday(closep) == 5)
nextday_idx <- pmin(fridays + 1, nrows)
closef <- closep[fridays]
openn <- openp[nextday_idx]
...
### write your code here
pnls_weekend <- xts::xts(
  zoo::coredata(openn) - zoo::coredata(closef),
  order.by = zoo::index(openn)
)
wealthv <- cbind(retp, pnls_weekend)
colnames(wealthv) <- c("SPY", "weekend")
...
### write your code here
sharpe_ratio <- sqrt(252) * colMeans(wealthv) / apply(wealthv, 2, sd)
sortino_ratio <- sqrt(252) * colMeans(wealthv) / apply(wealthv, 2, function(x) sd(x[x < 0]))
ratios <- rbind(Sharpe = sharpe_ratio, Sortino = sortino_ratio)
print(ratios)
...
### write your code here
dygraphs::dygraph(xts::xts(apply(wealthv, 2, cumsum), order.by = zoo::index(wealthv)),
                  main = "Weekend Calendar Strategy vs SPY")
