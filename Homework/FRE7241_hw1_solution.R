#################################
### FRE7241 Homework #1 Solution due at 6PM Tuesday January 27, 2026
#################################
# Max score 130pts
...
for (t in 2:nrows) {
  ...
  stocki[t] <- (1 + retp[t, "VTI"])*stocki[t-1]
  stockv[t] <- stocki[t] - margv[t-1]
  portfv[t] <- stockv[t] + (1 + retp[t, "TLT"])*bondv[t-1]
...
sim_cppi <- function(bfloor, coeff) {
  ...
  # Calculate the Sharpe of CPPI wealth
  retcppi <- rutils::diffit(portfv)
  sqrt(252)*mean(retcppi)/sd(retcppi)
}  # end sim_cppi
...
sharper <- sapply(bfloors, sim_cppi, coeff=2)
...
sharper <- sapply(coeffv, sim_cppi, bfloor=10)
...
retsub <- retp[endd[NROW(endd)-1]:endd[NROW(endd)], symbolv]
...
capml <- lapply(2:NROW(endd), function(it) {
  retsub <- retp[endd[(it-1)]:endd[it], symbolv]
  capmd <- PerformanceAnalytics::table.CAPM(Ra=retsub[, -1],
                                            Rb=retsub[, 1], scale=252)
