...
pnll <- lapply(rutils::etfenv$symbolv, function(symbol) {
  ...
  closep <- rutils::lagit(closep, lagg=1, pad_zeros=FALSE)
  reton <- (openp - closep)
  colnames(reton) <- symbol
  reton
})  # end lapply
...
symboln <- names(pnlf)[1]
pnls <- get(symboln, pnll)
...
colnames(wealthv) <- c("daily", "overnight")
endw <- rutils::calc_endpoints(wealthv, interval="weeks")
dygraphs::dygraph(cumsum(wealthv)[endw], main=paste("Wealth of", symboln, "Overnight Strategy")) %>%
...
desm <- na.omit(desm)
symbolv <- c("Overnight", "XLK", "Treynor")
colnames(desm) <- symbolv
...
formulav <- as.formula(paste(symbolv[1],
  paste(symbolv[-1], collapse="+"), sep=" ~ "))
...
resids <- (desm$Overnight - regmod$coeff["XLK"]*retp)
resids <- regmod$residuals
plot.default(x=retp, y=resids, xlab="XLK", ylab="residuals")
title(main="Treynor-Mazuy Test\n for Overnight Strategy vs XLK", line=0.5)
coefreg <- summary(regmod)$coeff
fitv <- regmod$fitted.values - coefreg["XLK", "Estimate"]*retp
...
resids <- (desm$Overnight - regmod$coeff["XLK"]*retp)
resids <- regmod$residuals
plot.default(x=retp["/2019"], y=resids, xlab="XLK", ylab="residuals")
title(main="Treynor-Mazuy Test
      for Overnight Strategy vs XLK
      Up to 2019", line=0.5)
coefreg <- summary(regmod)$coeff
fitv <- regmod$fitted.values - coefreg["XLK", "Estimate"]*retp["/2019"]
tvalue <- round(coefreg["Treynor", "t value"], 2)
points.default(x=retp["/2019"], y=fitv, pch=16, col="red")
text(x=0.0, y=0.8*max(resids), paste("Treynor test t-value =", tvalue))
